...
export default function SectionType4({ section }: { section: Section }) {
  return (
    <div
      id={section.type}
      className="description-container-bg text-white mt-5 md:mt-0 p-5 md:p-6 rounded-md"
    >
      {section.values &&
        ...
        section.values.map((data, inx) => (
          <div key={inx} className="flex justify-between">
            <div className="">
              ...
              <p className="font-semibold text-[16px] md:text-[20px] mt-4 w-full md:w-[50%]">
                {data.title}
              </p>
              ...
              <div className="mt-5">
                <Link
                  ...
                  className="mt-6 bg-green-500 text-[14px] md:text-[16px] md:px-7 px-5 py-2 md:py-2 rounded-md border-b-4 border-b-green-800"
                >
                  {data.cta?.text}
                </Link>
              </div>
